...
(function () {
  ...
  KV.scene({
    ...
    build: function (root, api) {
      ...
      /* Ключи подобраны так, чтобы hash % n давал ровную раскладку при ЛЮБОМ
         числе партиций 1..6 — иначе «здоровый» режим выглядел бы перекошенным.
         uid-999 (горячий) при каждом n попадает в свою партицию. */
      var USERS = ["uid-167", "uid-176", "uid-532", "uid-650", "uid-821"];
      var HOT = "uid-999";
      var SEED = 20260914;
      ...
      function render() {
        ...
        else if (cap >= cfg.rate) verdict = "запас <b>+" + (cap - cfg.rate) + "/с</b>, lag держится у нуля";
        else verdict = "дефицит <b>−" + (cfg.rate - cap) + "/с</b> — примерно на столько lag растёт каждую секунду";
        ...
        stage.say("продюсер <b>" + cfg.rate + "/с</b> · группа <b>" + work + " × " + cfg.speed +
      ...
      root.appendChild(ui.prose(
        ...
        "Седьмой консьюмер на шести партициях прибавляет ровно ноль: [[партиция|партицию]] в группе читает максимум один.</li>" +
        ...
        "<li><strong>[[ребаланс]] — стоп-кадр для всей группы.</strong> Пока партиции раздаются заново, не читает никто, " +
        ...
        "44 клетки — в настоящей Kafka такой предел задаёт <code>retention.bytes</code>. И главное: ползунок «партиций» " +
      ...
      root.appendChild(ui.takeaway([
        "Пропускная способность группы = <b>консьюмеры, которым достались партиции</b> × скорость каждого. Лишние сверх числа партиций просто стоят.",
