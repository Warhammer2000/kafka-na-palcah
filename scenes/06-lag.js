    ...
    build: function (root, api) {
      ...
      var btnUp = ui.btn("Поднять консьюмера", function () {
        ...
        stage.say("<b>Консьюмер поднялся.</b> Первым делом он спросил у Kafka свои позиции — <code>" + pos + "</code> — " +
          ...
          (catchUp
            ? "Пока разрыв большой, он читает на полной скорости (" + consumerRate() + "/с) и догоняет."
            : "Но ползунок чтения стоит на нуле: процесс поднялся, а разбирать он не начал — lag так и растёт со скоростью записи."));
      }, { variant: "read" });
      btnUp.disabled = true;
