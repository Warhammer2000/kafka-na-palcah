...
(function () {
  ...
  KV.scene({
    ...
    build: function (root, api) {
        ...
        say("<b>Выборы. Новый лидер — брокер " + nw.id + ".</b> " +
          (currentIsr
            ...
            : "Партиция стояла без лидера и дождалась брокера " + nw.id + " — <b>последнего, кто числился в ISR перед смертью</b>. " +
              ...
              (lost
                ? "Но лог у него короче: " + nw.log.length + " из " + oldLen + " записей — остальные до этой копии дойти не успели. "
                : "Лог у него полный, все " + oldLen + " " +
                  util.plural(oldLen, "запись", "записи", "записей") + " на месте. ")) +
          ...
          (lost
            ...
            : currentIsr
              ...
              : "<b>Не потеряно ничего</b>: партиция вернулась в работу с полным логом."));
      }
      ...
      var d2 = ui.range({
        label: "задержка брокера 2", min: 100, max: 2000, step: 100, value: 250, unit: "мс",
        onInput: function (v) { setDelay(1, v); }
      });
      var d3 = ui.range({
        label: "задержка брокера 3", min: 100, max: 2000, step: 100, value: 250, unit: "мс",
      ...
      function setDelay(idx, v) {
        ...
        b.delay = v;
        if (!b.alive) {
          say("Брокер " + b.id + " сейчас лежит — копировать ему нечего. Задержка " + fmt1(v / 1000) +
            " с запомнена и заработает, когда он вернётся в кластер репликой.");
          return;
        }
        if (b.leader) {
          say("Брокер " + b.id + " сейчас <b>лидер</b>: он источник истины и ни за кем не копирует, " +
            "поэтому отставать ему не от чего и из ISR его не выкидывают. Задержка " + fmt1(v / 1000) +
            " с пригодится, когда он снова станет репликой — например, после возвращения старого лидера.");
          return;
        }
        if (!leader()) {
          say("Брокер " + b.id + " жив, но у партиции сейчас <b>нет лидера</b> — копировать не у кого. " +
            "Задержка " + fmt1(v / 1000) + " с заработает, как только лидер выберется.");
          return;
        }
        say("Брокер " + b.id + " копирует примерно одну запись за " + fmt1(v / 1000) + " с. " +
          (!b.isr
            ? "Из ISR его уже выкинули: чтобы вернуться в список, ему надо <b>догнать лидера</b> — " +
