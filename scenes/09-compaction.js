    ...
    build: function (root, api) {
      ...
      var logPanel = ui.panel("Лог топика users · партиция 0",
        ...
        el("div", { style: { "margin-top": "2px" } }, ui.legend([
          ...
          { color: "var(--line)", label: "зачёркнутая клетка в пунктирной рамке — выброшена уплотнением" }
        ])));
      ...
      function paint() {
        ...
        if (policy !== "compact") {
          ...
          snapNote.innerHTML = drop
            ? "<b>Уплотнение уже прошло:</b> " + drop + " " +
              util.plural(drop, "запись выброшена", "записи выброшены", "записей выброшено") +
              ", и снимок держится на том, что уцелело. " +
            ...
            : "<b>Сейчас этот снимок держится на честном слове:</b> он верен только потому, " +
