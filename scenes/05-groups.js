    ...
    build: function (root, api) {
      ...
      function readBilling() {
        ...
        if (count) {
          ...
          var tail;
          if (workers < 2) {
            tail = ": всё это забрала одна копия — свои партиции она обходит в одном цикле опроса, одну за другой. " +
              "Параллелизма в группе сейчас нет: он появляется только тогда, когда партиции разобраны РАЗНЫМИ копиями.";
          } else if (solo) {
            tail = ": " + workers + " " + util.plural(workers, "копия работает", "копии работают", "копий работают") +
              " параллельно, и каждой досталось ровно по одной партиции — быстрее эта группа читать уже не сможет.";
          } else {
            tail = ": " + workers + " " + util.plural(workers, "копия работает", "копии работают", "копий работают") +
              " параллельно, но партиций больше, чем работающих копий, — кому досталось несколько, тот обходит их в одном цикле опроса, одну за другой.";
          }
          msg = "<b>billing читает.</b> " + order.map(function (mi) {
            ...
            util.plural(count, "запись", "записи", "записей") + tail;
        } else {
        ...
        if (finished.length && count) {
          msg += " " + (finished.length > 1
            ? "Партиции " + finished.join(", ") + " уже дочитаны до конца."
            : "Партиция " + finished[0] + " уже дочитана до конца.");
        }
        say(msg);
      }
      ...
      function applyCopies() {
        ...
        if (!d) {
          ...
          syncButtons();
          say((gone === 1
            ? "<b>Из списка убрали строку копии, которая уже упала.</b> Нового ребаланса нет, и это не поблажка стенда: " +
              "из группы она выбыла ещё на прошлом ребалансе, тогда же её партиции разобрали живые. "
            : "<b>Из списка убрали строки копий, которые уже упали.</b> Нового ребаланса нет, и это не поблажка стенда: " +
              "из группы они выбыли ещё на прошлом ребалансе, тогда же их партиции разобрали живые. ") +
            "Живых копий по-прежнему <b>" + nowLive + "</b>, раздача не меняется.");
          return;
        }
