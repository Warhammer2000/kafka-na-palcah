  ...
  KV.scene({
    ...
    build: function (root, api) {
      ...
      function doRebalance() {
        ...
        vSet("<b>Ребаланс пошёл.</b> Партиции отобраны у всей группы — стенд показывает обычную, eager-стратегию. " +
        ...
        api.timeout(1600, function () {
          ...
          vSet(before.length
            ? "Назначение роздано заново. До ребаланса lag рос на <b>" + plist(before) + "</b> — " +
              (suspect >= 0
                ? (before.length > 1 ? "их читал <b>" : "её читал <b>") + podName(suspect) + "</b>. Смотрю, за чем он поедет…"
                : "читали их разные поды. Смотрю, что сдвинется после переезда…")
      ...
      function finishMeasure() {
        ...
        if (!after.length) {
          ...
          head = after.length > 1
            ? "<b>Lag остался на тех же партициях: " + plist(after) + ".</b> Партиции переехали к другим подам — болезнь переехала вместе с ними."
            : "<b>Lag остался на той же партиции: " + plist(after) + ".</b> Партиция переехала к другому поду — болезнь переехала вместе с ней.";
          body = hot
