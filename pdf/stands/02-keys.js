...
module.exports = {
  ...
  build(ctx, L, page) {
    ...
    L.wrapText(page, "Хеш считается от байтов ключа, поэтому один и тот же ключ всегда даёт один и тот же номер: партиция ключа не меняется никогда. Разные ключи вполне могут съехаться в одну партицию — хеш обещает стабильность адреса, а не разные адреса. Сама Kafka считает хеш алгоритмом murmur2; здесь для наглядности взят crc32, арифметика та же.", {
...
function s2_key(j) {
  ...
  s2_stats();
  txt("s2_say", "Ключ тот же — партиция та же: все события " + s2_NAME[j] + " лежат в партиции " + p + " подряд, и только там порядок гарантирован.");
}
...
function s2_hot() {
  ...
  var p = s2_P[j];
  var added = 0;
  for (var i = 0; i < 6; i++) { if (s2_put(p, s2_ST[p][j])) added++; }
  s2_formula(j);
  s2_stats();
  if (added === 0) {
    txt("s2_say", "Партиция " + p + " уже забита этим ключом до края стенда — перекос виден по счётчикам. Жми «Сброс».");
    return;
  }
  var other = "";
  ...
  txt("s2_say", "Перекос: все события одного ключа легли в партицию " + p + ". Там " + s2_cnt[p] + " записей, в других " + other + " — они простаивают.");
}
...
function s2_auto() {
  ...
  if (s2_put(p, s2_ST[p][j])) {
    s2_full = 0;
    s2_stats();
    txt("s2_say", "Показ: " + s2_NAME[j] + " снова уходит в партицию " + p + " — ключ тот же, значит и партиция та же.");
  } else {
    s2_full++;
  }
