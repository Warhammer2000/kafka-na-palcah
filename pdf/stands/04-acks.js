...
function s4_render() {
  ...
  if (s4_alive) {
    ...
    if (s4_isrN === 3) {
      ...
      txt("s4_st1", "МЁРТВ, но в ISR");
      txt("s4_isrv", "ISR = [1] — лидера нет");
      txt("s4_st2", "вне ISR");
      txt("s4_st3", "вне ISR");
    }
  }
...
function s4_doKill() {
  ...
  if (s4_n === 0) {
    ...
    s4_setVd(1);
    txt("s4_say", "Лидер умер. Копии есть на брокерах 2 и 3, но оба вне ISR: без unclean-выборов партиция остаётся без лидера — данные целы, но недоступны. Пустым ISR при этом не становится: в нём так и висит мёртвый брокер 1.");
  } else {
    s4_setVd(2);
