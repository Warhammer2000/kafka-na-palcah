...
module.exports = {
  ...
  build(ctx, L, page) {
    ...
    L.tag(page, fonts, "min.insync.replicas — минимум копий", RX, 336, C.read);
    [1, 2, 3].forEach((v, i) => {
    ...
    L.stack(ctx, page, "s4_vd", [
      ...
      { fill: C.bad, border: C.bad, caption: "ТИХАЯ ПОТЕРЯ — продюсер получил ok, данных нет", captionColor: C.white, size: 13 },
...
var s4_alive = 1;
var s4_rep = [0, 0, 0, 0, 0, 0];   /* доехала ли запись до реплик */
var s4_pending = -1;   /* запись, которая ждёт репликации на следующем такте */
var s4_manual = false;
var s4_step = 0;
...
function s4_render() {
  ...
  if (s4_alive) { txt("s4_st1", "лидер, жив"); } else { txt("s4_st1", "МЁРТВ"); }
}
...
function s4_doWrite() {
  ...
  if (s4_a === 2 && s4_isrN < s4_m) {
    s4_setVd(3);
    txt("s4_say", "acks=all ждёт всех ИЗ ISR, а там один лидер: 1 < min.insync = " + s4_m + ". Брокер отказал: NotEnoughReplicasException. Ничего не записано.");
    return;
  }
  var i = s4_n;
  show("s4_a" + i, 1, 2);
  ...
  if (s4_a === 0) {
    ...
    txt("s4_say", "acks=1: лидер записал #" + (i + 1) + " к себе и сразу ответил «ok». Реплики о ней пока не знают.");
  } else if (s4_isrN === 3) {
    txt("s4_say", "acks=all: лидер записал #" + (i + 1) + " и ждёт всех ИЗ ISR. В ISR трое — через такт копии появятся у брокеров 2 и 3.");
  } else {
...
function s4_doKill() {
  ...
  if (s4_n === 0) {
    ...
    s4_setVd(1);
    txt("s4_say", "Лидер умер, но каждая запись успела уехать в реплики. Новым лидером станет брокер 2 — данные целы.");
  } else {
    s4_setVd(2);
    txt("s4_say", "Лидер умер. Записей, живших только на нём: " + lost + ". Продюсер получил на них «ok» и ошибки не увидит — это тихая потеря.");
  }
}
...
function s4_pump() {
  ...
  show("s4_b" + i, 1, 2);
  show("s4_c" + i, 1, 2);
  txt("s4_say", "Брокеры 2 и 3 подтвердили запись #" + (i + 1) + ": три копии. Теперь смерть лидера ей не страшна.");
}
